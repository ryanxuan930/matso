...
## 追加：R 層 + P3（同批完成）
- **R 層入 UI**：tick 長度、真實節奏壓縮比、通聯重算間隔、AI 決策心跳、AI 落單上限。
  機制與 P1 相同（runner 啟動時讀），故**進行中的局同樣不受影響**。
  `tick_rate_ms` 下限夾 1000（實測給 10 → 夾成 1000）；`pace_compression` 負值退預設。
  預覽端的 ETA 也改讀同一個 `tick_rate_ms`（紀律 2）。
- **P3 唯讀項標註**：系統資訊每一項後面標「需重啟 core」，並把說明改成
  「在畫面上改也無處落地」——避免使用者以為改了就會生效。

## 未做（P2，見 PARAMS.md 分期）
**刻意不做 P2 的韌性/逾時參數**（gRPC deadline、health interval、circuit breaker 門檻）：
`TerrainClient` 是每次請求由 DI 建構的，且這組值直接影響**物理預檢閘門**——
一個過短的 deadline 會讓合法命令被誤拒、過長則拖垮 p99。這需要自己的測試與觀測，
不該和「兵推行為參數」混在同一批交付。另開卡處理。
