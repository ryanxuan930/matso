...
│   COP 地圖 │ 指令下達 UI │ 白軍控制台 │ AAR 儀表板 │ 想定編輯器      │
└──────────────┬────────────────────────────────┬─────────────────────┘
               │ REST (指令/查詢)                │ WebSocket (狀態推播)
┌──────────────▼────────────────────────────────▼─────────────────────┐
...
│ (DTED/   │ │ (CWA data) │ │ (CV, 非AI) │ │ LoRA hot-swap          │
...
- **Golden Replay 保證**：`replay(session_id)` 讀取 Event Ledger 中的指令序列重新執行，最終狀態 hash MUST 與原始執行一致（CI 強制驗證，見 §19）。
...
- **RAG**：Qdrant 向量庫，collection 按領域分割（`doctrine_blue`, `doctrine_red`, `equipment_specs`, `terrain_analysis`, `historical_ops`）。入庫管線：markdown/PDF → 語意切塊（512 tokens, overlap 64）→ bge-m3 嵌入（中英雙語）→ 附 metadata（來源、密級、版本）。
...
| G4 | **IHL/ROE 合規檢查**：規則引擎比對目標清單與保護目標資料庫（醫院、文化資產、平民區 hex 標記）；比對想定 ROE（如「不得越過某線」「禁用某類武器」） | **硬性阻擋**，事件升級 White Cell 人工裁定 |
| G5 | **引用查核**：`cited_documents` 必須存在於 RAG 庫且相似度 > 閾值（防捏造引用） | 標記 `citation_unverified`，降信心度 |
...
```
scenario.yaml          # 元資料：名稱、地圖範圍(bbox)、模式(realtime/WEGO)、tick 設定、勝利條件
orbat/blue.yaml        # 藍軍戰鬥序列（巢狀單位樹 + 裝備 + 初始位置）
orbat/red.yaml
...
- **每一條 API 都要通過 faction-scope 檢查**：例如 RED 玩家呼叫 `GET /units` 只會得到紅軍單位 + 紅軍情報所見的藍軍 contact。此過濾 MUST 在後端做，前端過濾不可信。
...
| **可用性** | 推演進行中 Core 可用性 99.5%；崩潰後 5 分鐘內可由 checkpoint+ledger 復原（RPO = 0 事件遺失，RTO ≤ 5min） |
