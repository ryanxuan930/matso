...
| WP-B1 演習專案（a/b/c）✅ + WP-B4 參數簽證 ✅ | SPEC_V2 §6 WP-B1/B4 | `Exercise` 階段機 + 專屬 audit 表 + 整備勾稽 + 掛/卸 session（B1a）；歸檔封包 + 銷毀模式 + 修好刪除推演的資料殘留（B1b）；lobby 演習分頁（B1c）；`ParameterSeal` + 三條寫入路徑 403 + 開局雜湊比對（B4） | ✅ pytest 1650（+54）、mypy 244、ruff、OpenAPI、schema-sync 23 tables / 222 columns、前端兩閘門綠；golden 未動。**刪掉契約裡的 `/sessions/{id}/lifecycle` 規格殘骸**（無 security/schema/實作）。三個與規格不同的裁決：簽證是明示動作（做成 EXECUTION 副作用會死鎖）、鎖住的是全域（那兩個端點不帶 session_id）、`PUT /system/config` 選擇性拒絕。順手修掉 `delete_session` 的資料殘留與 `ApiError` 丟失 details。worklog: exercise-lifecycle.md、parameter-seal.md |
| WP-A2 收尾：任務令接進活執行期 ✅ | SPEC_V2 §6 WP-A2 | `engine/mission_wiring.py` 的 `LiveMissionPlanner`（撈令→評估→送子令→進度寫回令）+ sim_runtime 傳進 Kernel（取代 NoOp）+ `_hydrate` 補 `to_h3` + `MISSION_SUBORDER_REJECTED` 痕跡 | ✅ pytest 1781（+9）、mypy 253、ruff、schema-sync、前端兩閘門綠；golden 未重錄。**使用者回報「任務功能異常」查出兩個獨立斷點**：planner 從沒被傳進 Kernel（`MissionPlanner` 全 repo 只有 Protocol + NoOp 兩個引用）；MOVE 子令因缺 `to_h3` 一律被驗證層打回且例外被 INFO log 吞掉。golden 抓不到（自帶純記憶體測試 planner）。活系統實測：2 道卡住的任務令轉 EXECUTING 並產出 5 道子令。worklog: mission-orders.md |
| WP-C4c 煙幕 ✅ | SPEC_V2 §6 WP-C4 | `adjudication/obscurants.py`（純幾何；`blocks_los` **簽名裡沒有陣營**＝雙面性的結構保證）+ `engine/smoke_wiring.py`（煙存 MapFeature + 逐 tick 快取）+ 兩條 LOS 疊加 + `ammo_type=SMOKE` 發煙任務 | ✅ pytest 1844（+13）、mypy 258、ruff、buf、前端兩閘門綠；**golden 未重錄、無 DB migration**（煙重用 MapFeature）。煙是布林遮蔽不是係數；雷達穿得過煙；發煙不抽落點散布（會擾動 RNG 串流卻不改變可觀測結果）。5 個突變轉紅——**髒資料那條第一版是假的**（一列同時兩種壞法，拿掉任一 guard 都綠）。未竟：風向漂移（weather 無風場）、前端畫煙、發煙下令 UI。worklog: obscurants.md |
| WP-E4 監控落地 ✅ | SPEC_V2 §6 WP-E4 | `app/metrics.py`（自寫 exposition、零相依）+ `GET /metrics` + Kernel/broadcaster/worker/RoleManager 埋點 + `ops/monitoring/`（scrape/告警/3 份儀表板）| ✅ pytest 1934（+13）、mypy 265、ruff 綠；**golden 未重錄**（量測不入帳本）。**使用者裁示不加容器**（Grafana 3000 撞前端 + air-gapped）。指標**不帶 session 標籤**（基數爆炸 + /metrics 不驗身分）。⚠ **直方圖累積做了兩次**，分位數是錯的——曲線仍單調遞增所以看起來合理，印樣張才發現。5 個突變全數被抓。未竟：`io_latency`/`ai_workers` **定義了但沒有寫入端**。worklog: monitoring.md |
| WP-E2 認證強化 ✅ | SPEC_V2 §6 WP-E2 | `RevokedToken` 表 + refresh 輪替/重用偵測 + 帳號鎖定（5 次/15 分）+ `needs_rehash` 接線 + logout 真的撤銷 | ✅ pytest 1921（+12）、mypy 264、ruff、schema-sync（24/232）、前端兩閘門綠。**登出在此之前是 no-op**（註解自己寫著「不維護黑名單」）。鎖定時回**同一個**錯誤（分開回會洩漏帳號存在）。6 個突變全數被抓——**其中一條靠突變才發現「只驗第一次不拋」殺不掉「把空 jti 寫進撤銷表」**。未竟：完整家族撤銷、撤銷表無清理排程。worklog: auth-hardening.md |
| WP-F1 SPEC_INGEST 最小切片 ✅ | SPEC_V2 §6 WP-F1 | `load_bge_m3()`（惰性、非硬相依、取不到回 None）+ `describe_embedder()` 降級標示 + ingest `--embedder bge-m3` + `corpus_manifest` + `evals/retrieval.py` hit@k | ✅ pytest 1909（+12）、mypy 264、ruff 綠。**降級三處同時標示**（UI 旗標/CLI stdout/manifest）——無聲降級會讓人信任它的引用。**`total=0` 不算通過**（「沒有東西可測」≠「測了而且過了」）。5 個突變全數被抓——**其中一條靠突變才發現 fake retriever 自己先切了 k**。未竟多為部署決定：套件與模型檔未納入 air-gapped 資產、PDF parse 未接 CLI、QA 對還沒有任何一批。worklog: rag-ingest.md |
| WP-F3 RoleManager/AIInvocationLog 接入活執行期 ✅ | SPEC_V2 §6 WP-F3 | `AIRequest.system_prompt` 覆寫 + decider 注入 RoleManager + `bind_session` + `ai_loop_wired` 修正 | ✅ pytest 1897（+8）、mypy 263、ruff、前端兩閘門綠；golden 未重錄。**同一個病的第三例**（RoleManager/InvocationLogWriter 非測試引用是 0）。**路由不可改掉 prompt**——ReplayClient 按 prompt 雜湊重播，改了會讓所有已錄自主場次失效；有測試逐字比對兩條路徑的 messages。4 個突變全數被抓。未竟：佇列批次未生效（走單發 invoke）、`guardrail_result` 仍 not_evaluated、AAR 不顯示稽核。worklog: ai-audit.md |
| WP-C7.3 修復與人員補充（C7 收尾）✅ | SPEC_V2 §6 WP-C7 | `engine/refit_wiring.py`（三個前提、遭襲中斷、按經過 tick 累積、`REFIT_*` 事件）+ SimParams `repair_per_day`（0＝中性）| ✅ pytest 1889（+11）、mypy 263、ruff、schema-sync 綠；golden 未重錄、無 migration、無契約變更。**驗收條文達成**：修復中的單位遭襲即中斷整補。遭襲判定用**壓制度**不用戰力下降（後者慢一拍）；中斷時計時歸零（否則等於一邊挨打一邊修車）；第一個 tick 只計時不修。**7 個突變全數被抓**。未竟：人員補充未與裝備修復分開（待 #48）、想定不能設修復率、前端不顯示。worklog: refit.md |
| WP-C7.2 再訂購水位與補給線 ✅ | SPEC_V2 §6 WP-C7 | `engine/supply_points.py`（`MapFeature(kind=SUPPLY_POINT)`、拉式撥交、部分撥交、`destroy_at`）+ `auto_resupply` 掛 pre_tick + 面射擊命中摧毀補給點 | ✅ pytest 1878（+12）、mypy 262、ruff 綠；golden 未重錄、無 migration、無契約變更。**驗收條文達成**：打掉補給點後下游水位不再回升（端到端測試）。撥交是「拉」不是「推」（推送要全知，且讓「補給線被切斷」無法表達）。6 個突變全數被抓——**其中一條靠突變才發現「斷言結果不等於斷言路徑」**。未竟：補給車運輸、審批鏈開關、補給點不能從想定建立。worklog: supply-lines.md |
| WP-C7.1 補給類別最小集 ✅ | SPEC_V2 §6 WP-C7 | `adjudication/supply.py`（Class I/IX、`capacity<=0`＝未編制、斷補階梯）+ `engine/supply_wiring.py`（按經過 tick 補算）+ 射手效能接線 + SimParams 消耗率 | ✅ pytest 1866（+16）、mypy 261、ruff 綠；**golden 未重錄、無 migration、無契約變更**。**刻意不把 Class III/V 搬進新體系**（已有能用的模型，重寫只換到回歸風險）。缺鍵回空 dict 而非全 0；沒有實際消耗連時間戳都不寫；`write_levels` 排序（熱狀態進 state hash）。6 個突變全數被抓——**其中一條靠突變才發現測試碰不到它要驗的 guard**。未竟：LOGISTICS capacity 類別命名不一致、想定不能宣告 supply 初值、前端不顯示。worklog: supply-classes.md |
| WP-C4b 天氣逐 tick 刷新（C4 收尾）✅ | SPEC_V2 §6 WP-C4 | `engine/weather_wiring.py`（`WeatherCache`：每 N tick 重問、取不到沿用上一份、失敗也更新時間戳）+ `weather_for` 回呼 + `CellEffects` 補風場 + `obscurants.drift()` | ✅ pytest 1850（+10）、mypy 259、ruff、buf、schema-sync、前端兩閘門綠；**golden 未重錄、無 migration、無契約變更**。**規格的兩個前提查證後不成立**：插件 RPC 早就 tick-aware（是 core 永遠傳 0）、風場早就在契約裡（是 core 沒讀）。⚠ 風向是「來向」——煙要往 +180 度走，寫錯在畫面上完全合理。5 個突變全數被抓。未竟：`WeatherMode.REPLAY`、`drift()` 未掛查詢路徑。worklog: weather-tick.md |
| WP-C4a 晝夜與照明 ✅ | SPEC_V2 §6 WP-C4 | `adjudication/daylight.py`（純函數：跨午夜光照判定 + 三係數）+ `engine/daylight_wiring.py`（`LightClock`）+ `SensorProfile.night_capable` + `DetectionEnv.light_modifier` + 夜間行軍 + `day_night` 想定六層 | ✅ pytest 1831（+16）、mypy 256、ruff、schema-sync（23/225）、前端兩閘門綠；**golden 未重錄**（規格寫「重錄」是針對 C4b 的天氣）。6 個突變全數被抓——**其中跨午夜那條是靠突變測試才發現我的測試是假的**（選的案例白天區間沒有繞回去，改用曙光帶橫跨午夜才分得出對錯）。未竟：ILLUM 照明彈、前端晝夜呈現、C4b/C4c。worklog: daylight.md |
| WP-C9 友軍誤傷語意（含 C9a ROE 先決）✅ | SPEC_V2 §6 WP-C9 | `adjudication/fratricide.py`（純函數：關係矩陣判友軍 + 三態 ROE）+ `allow_fratricide` 想定開關（schema/loader/dump/prisma/model/session 六層）+ `FRATRICIDE` 事件（**每受害者一筆**）+ WS allowlist + AAR 書籤 + COP 文案；**先修 C9a**：四個正式路徑從沒注入 relations | ✅ pytest 1813（+22）、mypy 254、ruff、schema-sync（23/224）、buf、前端兩閘門綠；golden 未重錄。**規格說的「加個開關」不成立**：關的那一邊本來就破（人類可打盟軍、AI 反而擋得住）、三條路徑不對稱（FIRE_MISSION 根本沒有陣營檢查）、開關會順便放行中立、friendly_losses 用字串比較漏掉聯軍誤傷。7 個突變全數被抓。未竟：前端 affordance、AI 路徑、直射濺射（需先改武器契約）。worklog: fratricide.md |
| WP-C2 障礙工事與工兵裁決 ✅ | SPEC_V2 §6 WP-C2 | `adjudication/obstacles.py`（純函數：5 型別 × 速度倍率/觸雷率/破障工時）+ `engine/obstacle_wiring.py`（型別過濾/擲觸雷/壓制/ENGINEER 令執行）+ `Obstacle` 三個中性欄 + `obstacles_at()` 逐 tick 幾何 + 移動接線（速度倍率 + `MINE_STRIKE` 事件）+ `ENGINEER` 令型（BREACH/EMPLACE，含工時與預檢）+ SimParams 三係數 | ✅ pytest 1772（+34）、mypy 252、ruff、OpenAPI、schema-sync（23/223）、前端兩閘門綠；**5 個 golden 未重錄**。**規格的「移動 A* 完全無視它」查證後不成立**（#28 強穿耗損早就在），真正缺的是型別語意，SPEC_V2 已更正。中性做成**結構性**：`typed()` 在入口濾掉無型別標註 → 既有局一次幾何判定都不做、**一次 RNG 都不抽**。10 個突變全數被抓。**未竟：前端 ENGINEER/FORMATION 下令 UI**（後端通了但使用者點不到）。worklog: obstacles-engineer.md |
| WP-C3 乘駐車與隊形 ✅ | SPEC_V2 §6 WP-C3 | `adjudication/formation.py`（五種隊形 × 行軍速度/面殺傷暴露/火力正面 + 乘駐車三態）+ `engine/formation_wiring.py` + **一個** `FORMATION` 令型（收 MOUNT/DISMOUNT）+ `EnvSnapshot` 兩個新係數 + SimParams | ✅ pytest 1738、mypy 251、ruff、前端兩閘門綠；既有 golden 未重錄。**第一版把 `mounted` 缺鍵用 `bool()` 收成 False → 既有局命中率無聲下降 20%**（golden 與交戰單元測試都抓不到，錯在接線層）；改三態 `bool | None` 並在該層補測試（`9e10f58`）。未竟：前端切換 UI、2525 mounted 修飾、`mounted` 影響移動速度、載具毀損→乘員傷亡（待 #48）。worklog: mounted-formation.md |
| WP-C1 壓制與姿態系統 ✅ | SPEC_V2 §6 WP-C1 | `adjudication/suppression.py`（純函數：累積/衰減/姿態狀態機）+ `engine/suppression_wiring.py`（熱狀態 I/O）+ POSTURE 令執行 + 逐平台/聚合/**面射擊**三條裁決路徑 + SimParams 係數 + 契約/COP 壓制條與姿態徽章 + AI context + golden `suppression_defense_60` | ✅ pytest 1596（+41）、mypy 234、ruff、OpenAPI、schema-sync（20/197）、前端兩閘門綠；**既有 3 個 golden 未重錄**（中性預設）。兩個 modifier **從交戰真實化時代就恆為 1.0**；接線後跑驗收條文才發現砲兵路徑整條漏掉（面射擊不累積壓制、不看姿態、壓制半徑=殺傷半徑）。實測 20 發 155mm：DUG_IN 傷亡 0.64／露天 1.28、射擊效能修正 0.40。worklog: suppression-posture.md |
| WP-B2 MSEL 排程執行引擎（a/b/c）✅ | SPEC_V2 §6 WP-B2 | 條件 DSL 擴充（polygon/contact/manual/after_ticks_of/held_for/not）+ `msel_runtime`（純函數判斷）+ `msel_actions`（SPAWN_UNITS/MODIFY_UNIT/MESSAGE/PAUSE）+ 接上 Kernel trigger 槽 + 記憶進 checkpoint + 白軍待命注入命令通道與面板 | ✅ pytest 1555、mypy 232、ruff、OpenAPI、schema-sync（197 欄）、前端兩閘門綠；**golden 未動**。**動手前 MSEL 三層都是斷的**（回 LedgerEvent 改不了世界／傳 NoOp 從未被呼叫／想定的 msel 從不持久化）。SPAWN_UNITS 的單位 id 決定性派生（禁 uuid4）＋ resolver 惰性補查（否則增援下得了令卻一發打不出去）。worklog: msel-runtime.md |
| WP-C10.5 陣地變換 survivability_move ✅ | SPEC_V2 §6 WP-C10 | 想定開關（`WargameSession.survivabilityMove` JSON + contract + loader）+ 熱狀態任務計數 + `fires/displacement.py` 排程器（跑 `pre_tick`，**下 MOVE 令而非改座標**）+ 獨立 `"survivability"` RNG stream | ✅ pytest 1502（+16）、mypy 226、ruff、schema-sync（196 欄）、scenario schema 綠；golden 未動。**位移用令不用瞬移**的四個理由（會被 seed_combat_state 蓋回去／瞬移／同 tick 撞移動子系統／繞過唯一可達性閘門）寫在 worklog。牽引砲與 is_fixed 不排程。worklog: survivability-move.md |
| WP-C10.4b BDA 回報（帶迷霧誤差）✅ | SPEC_V2 §6 WP-C10 | `adjudication/bda.py` 純函數 + `BDA_REPORT` 事件（`observer_faction` 標受眾、`damage_calc`/`target_id` 皆 None、只給總量不給逐單位）+ 獨立 `"bda"` RNG stream + COP feed 文案 | ✅ **沒有觀測就沒有回報**（不是回報 0——0 會被讀成「打了但沒傷到」）；pytest 1486（+15）、mypy 224、ruff、前端兩閘門綠；**golden 未動**（獨立 stream）。誤差帶寫在事件裡（±30%）而非常數 confidence 標籤。worklog: bda-and-observation.md |
| WP-C10.4a 觀測判定 + 散布掛觀測者 ✅ | SPEC_V2 §6 WP-C10 | `ObserverVerdict`（OBSERVED/UNOBSERVED/**UNKNOWN**）+ `dispersion_mult` 進 `resolve_area_fire` + `feed_damage()` 關掉 damage 洩漏（WS feed **與 AI briefing** 兩處）| ✅ **驗收條件達成**（前觀死亡→散布加倍，端到端釘住）；pytest 1471（+24）、mypy 223、ruff、schema-sync 綠；**golden 未動**（`mult=1.0` 位元不變）。觀測候選重用 `_gather_targets()` 的熱狀態清單（死亡看得出來）、探測上限 8 次 + 15km、UNKNOWN fail open（故障不演成戰術事實）。worklog: bda-and-observation.md |
| WP-C10.3 火力計畫 + at_tick/on_call 排程 ✅ | SPEC_V2 §6 WP-C10 | `FirePlan`/`FirePlanTarget` 實體 + REST（列表**後端過濾陣營**）+ 排程器掛 `run_paced(pre_tick)` + COP 小工具；**不新增物理**——執行＝組 FireMissionPayload 走 `OrderService.submit` | ✅ pytest 1447、mypy 223、ruff、schema-sync（20 tables/195 欄）、OpenAPI 綠；前端 lint/typecheck 綠；新增 2 支 e2e（建計畫→on-call→指令列真的出現火力任務）。**規格的「MSEL 執行器複用」查證後不成立**（回 LedgerEvent 產生不了令／trigger 槽慢一 tick／活執行期是 NoOp），SPEC_V2 已一併更正。去重陷阱以變異測試驗過。worklog: fire-plan.md |
| WP-C10.2 面目標射擊（打座標）✅ | SPEC_V2 §6 WP-C10 | `OrderType.FIRE_MISSION` + `adjudication/area_fire`（CEP→Rayleigh 落點、齊射逐發抽落點、殺傷半徑內**敵我皆傷**）+ `engine/fire_wiring` 接線（`ChainedOrderSource`/`DispatchingAdjudicator`、獨立 RNG stream `area_fire`）+ 火協 gate 綁 FIRE_MISSION + COP 點地圖下火力任務 | ✅ pytest 1423、mypy 219、ruff、OpenAPI 綠；前端 lint/typecheck 綠；新增 2 支 e2e（含 `queryRenderedFeatures` 證明準星真的畫在畫布上）。**順手補三個洞**：核准單對 FIRE_MISSION 從未兌現（一張單可掛無限次令）、曲射武器類別散成兩份、逐發落點誤放在不入 hash chain 的 `detail`。友軍傷害那條以變異測試驗過不是虛假綠。worklog: area-fire.md |
| WP-C10.1 臨機火力申請（call-for-fire）✅ | SPEC_V2 §6 WP-C10 | `CALL_FOR_FIRE` 申請種類 + 觀測條件（共用交戰預檢的 PhysicsGateway，不另寫 LOS）+ 重用 B5.2 審批鏈；與 FIRE_SUPPORT 刻意分開（授權 vs 任務單） | ✅ 4 個 API 測試；pytest 1390、mypy 217、ruff、schema-sync 綠。**只做到申請受理**，自動生成 ENGAGE 令屬 C10.2。worklog: call-for-fire.md |
| WP-B5.3 曲射火協 gate ✅ | SPEC_V2 §6 WP-B5 | 想定開關 + `fire_request_id` + 預檢 gate + 令收下時兌現核准單；**不指名武器不得繞過**（與 ROE 的做法刻意不同） | ✅ 10 個測試（含未開關零行為變更、繞過洞、EXPENDED 不得重用、跨陣營/錯種類核准單）；pytest 1385、mypy 217、ruff、schema-sync（172 欄）、OpenAPI 綠。worklog: fire-approval-gate.md |
| WP-B5.2 C2 信文與申請-核覆 ✅ | SPEC_V2 §6 WP-B5 | `Message`/`Request` + 審批鏈 + 想定層配額（開局快照）+ WS 席位受眾（只能收窄）+ COP 信文小工具；切出 B5.3 火協 gate 與 B5.4 標繪分送 | ✅ pytest 1375、mypy 217、ruff、schema-sync（18 tables/171 columns）綠；前端三閘門綠；容器實測送信→申請→核准全鏈路（信文匣自動生成 REQUEST/APPROVAL 兩封）；e2e `.env`-free worktree 4 failed/18 passed 與前一卡逐條相同。worklog: c2-messaging.md |
| WP-B5.1 席位模型（seat_role）✅ | SPEC_V2 §6 WP-B5 第一張卡 | `SessionParticipant.seatRole`（NULL＝沿用角色權限，既有局零遷移）+ 下令權單一 registry （`core/app/seats`）+ 新錯誤碼 `ORDER_SEAT_DENIED` + 名冊席位指派 UI | ✅ 契約先行驗證通過、prisma migrate（145 欄）、pytest 1305、mypy 214、ruff 綠；前端 lint/typecheck/build 綠；**容器實測**：migration 後 9 筆參與者全 NULL、設 S3_OPS 落庫、改回空還原為 NULL。worklog: seat-model.md |
| WP-D6.1 AAR 地圖重播（量綱修正先行）✅ | SPEC_V2 §6 WP-D6 第 1 項 | `reconstruct_states` 從未被 API 接線，底下 5 個既有錯全修（量綱、聚合戰損歸帳、位置來源、tick 非單調、權威值被覆寫）；契約先行新增 `/aar/replay/states`（底本 + 逐 tick 差異）；前端 `useAarReplay` + 地圖重播 + 播放/倍速/書籤 | ✅ pytest 1299、mypy 213、ruff、schema-sync 綠；前端 lint/typecheck/build 綠；**新增 2 支真 Playwright e2e**（斷言 `queryRenderedFeatures > 0`，即符號真的畫出來而非只有資料）。worklog: aar-map-replay.md |
