...
> **第一次操作這套系統的人請直接看 [§0 快速上手](#0-快速上手第一次操作這套系統)**
> ——啟動、登入、跑第一場推演、看懂畫面上的數字。不需要讀懂程式碼。
>
> 第 1 節之後是**系統現況的完整解剖**（給開發者與 AI agent）：架構、功能、每個檔案的用途與相互關係。
...
0. [**快速上手（第一次操作這套系統）**](#0-快速上手第一次操作這套系統)
...
> 這一節寫給**操作者**（統裁、指揮官、參謀），不需要讀懂程式碼。
> 底下第 1 節之後才是給開發者與 AI agent 的系統解剖。

### 0.1 這套系統能做什麼

MATSO 是一套**營旅級陸戰兵棋推演系統**。它與一般戰爭遊戲最根本的差別是：
**AI 永遠不裁決物理事實**。命中與否、看不看得見、走不走得到，一律由確定性的物理引擎計算；
LLM 只能像參謀一樣**提出命令**，那些命令與人類下的令走**完全同一條**驗證、預檢、交戰規則閘門。

因此同一份想定＋同一顆亂數種子，跑一百次會得到**逐位元相同**的結局——這是它能拿來做
分析與檢討的前提。
...
| 面向 | 做得到什麼 |
|------|-----------|
| **想定** | 劇本編輯器建立戰場範圍、多陣營與敵我關係矩陣、ORBAT 編組樹、交戰規則（ROE）、禁射區、狀況處置腳本（MSEL）、勝負條件 |
| **編組** | 逐單位的番號、編制層級、兵科（決定地圖符號）、戰力與人數、裝備與彈藥；白軍可開放各軍自編 |
| **機動** | 地形／坡度／道路調速、A* 繞開不可通行區、油料消耗與斷油拋錨、行軍耗損、強行軍、夜間行軍、隊形與乘駐車、行軍縱隊間隔 |
| **火力** | 逐武器的聯合兵種加總、彈藥逐發扣減、面目標射擊（打座標）與散布、觀測修正、彈著損害評估（BDA）、火力計畫與預劃目標、火力支援協調申請 |
| **戰場環境** | 逐格天氣（能見度／機動／無線電衰減／風）、晝夜、煙幕（隨風漂移）、五型障礙與工兵破障設障 |
| **情報與通信** | 逐單位感測器掃描、迷霧投影（看不到就是看不到）、通聯鏈路預算（距離／地形遮蔽／天氣）、斷聯時的位置凍結 |
| **後勤** | 四類補給水位、每日消耗、補給點自動撥交、戰損整補 |
| **指揮體系** | 席位分工（指揮官／S2／S3／FSO／S4／觀察員）、C2 信文、申請—核覆鏈與配額 |
| **統裁** | 白軍視角切換、暫停／續行／回放到任一快照點、狀況注入、單位參數軟裁決、參數凍結簽證 |
| **AI 對抗** | 指定任一陣營交給 LLM 指揮，逐陣營設任務與目標；AI 的令一樣過護欄與預檢 |
| **檢討** | 逐事件帳本（雜湊鏈防竄改）、時間軸重播、敘事型 AAR 報告並查核引用真偽 |

### 0.2 啟動系統

```bash
# 1. 一次性：離線產生 gRPC 程式碼 + 安裝 Python 相依
uv run python ops/tools/gen_proto.py
uv sync

# 2. 建資料庫結構（MariaDB 對外走 3307）
cd db && npx prisma migrate deploy && cd ..

# 3. 起全部服務（資料庫／Redis／向量庫／地形／天氣／通信／後端／前端）
cd ops/compose && docker compose up -d --wait

# 4. 建第一個可登入的帳號（統裁權限）
SEED_USERNAME=commander SEED_PASSWORD=exercise SEED_ROLE=EXERCISE_DIRECTOR \
  uv run python ops/tools/seed_dev_user.py
```

開瀏覽器到 **<http://localhost:3000>**，以上面建的帳號登入。

> ⚠ **前端一律跑 container，不要 `npm run dev`。** 兩條路徑的底圖設定來源不同
> （dev server 讀 `platform/.env`、container 讀 compose 注入），並行過會出現「底圖突然不見」。
> 改了前端要看效果：`cd ops/compose && docker compose up -d --build frontend`
> （**必須 `up`**——單獨 `build` 不會換掉正在跑的容器）。

### 0.3 角色與看得到什麼

| 角色 | 定位 | 看得到 |
|------|------|--------|
| **演習總監（EXERCISE_DIRECTOR）** | 演習的最高權責者 | 全知；可切任一陣營視角、控制時間、注入狀況 |
| **白軍統裁（WHITE_CELL_STAFF）** | 導演與裁判 | 同上 |
| **指揮官（COMMANDER）** | 帶一個陣營 | **只有自己陣營＋盟軍**的真實位置；敵情只有偵獲到的 |
| **參謀（STAFF）** | 佔一個席位 | 同指揮官，但**能下的令依席位而定** |
| **觀察員（OBSERVER）** | 隨隊觀摩 | 唯讀 |
| **分析官（ANALYST）** | 事後檢討 | 帳本與 AAR |
...
|------|---------|
| 指揮官 | 全部 |
| 作戰官（S3） | 移動、任務、姿態、隊形／乘駐車、工兵作業 |
| 火力支援協調官（FSO） | 交戰、火力任務 |
| 後勤官（S4） | 補給撥交 |
| 情報官（S2）／觀察員 | 唯讀 |

> 未指派席位＝沿用角色本身的權限（不是「不能下令」）。這是刻意的：
> 上線時既有參與者的席位都是空的，把空席當成禁止會讓進行中的演習立刻鎖死。

### 0.4 跑第一場推演（十分鐘）

1. **選一份想定**
   首頁「推演局」→ 下拉挑一份範例想定（`scenarios/examples/` 有教學排級、營級防禦、聯合防禦三份）
   → 按「從想定建立」。系統會建立戰場、部隊編組、敵我關係、交戰規則與狀況處置腳本。

2. **指派參與者**
   在該局按「參與者」→ 把帳號指到陣營與角色，需要時再指席位。
   要限制某人只能指揮特定幾支部隊，勾「限指揮單位」。
   *（想單人試跑就跳過這一步——總監本來就看得到全部。）*

3. **進入戰場（COP）**
   點推演局名稱進入。畫面左側是部隊清單，中央是地圖，右側與下方是可拖曳的浮動面板
   （指令、戰況事件、C2 信文、火力計畫、圖層、地圖標繪…可從工具列開關）。

4. **下第一道令**
   在地圖或清單點一支部隊 → 下令面板選令型：
   - **移動**：點地圖設目的地（或逐點點擊建自訂路徑）。送出前會出現**路徑試算**——
     距離、需要幾個 tick、油耗、行軍耗損、是否穿越不可通行地形。
     這份試算與實際執行走**同一份**機動規則，看到什麼就是會發生什麼。
   - **交戰**：選目標單位；持兩種以上武器時預設「聯合火力」（全武器一起打），
     也可以指定單一武器與火力政策。
   - **火力任務**：點地圖設落點（打座標，不是打單位）——**殺傷半徑內敵我一律受影響**。
   - **任務**：下的是「奪佔／防守／掩護幕／行軍」這種任務，由確定性的分解器逐步展開成低階令。

   送出後看**指令面板**：令會走 `已收下 → 已驗證 → 執行中 → 已完成`。
   被擋下來時，**預檢會逐項告訴你為什麼**（射程、視線、彈藥、交戰規則、禁射區…）。

5. **看戰況**
   **戰況事件**面板是即時的：交戰結果、抵達、觸雷、斷油、通聯變化、砲擊評估都會逐條出現。
   單位資訊卡顯示戰力、平台數、人數、戰備狀態、通聯、壓制度、姿態、逐武器彈藥。

   > **「戰備」與「血量」是兩件事**：血量是作戰效能百分比，戰力掉到三成以下就顯示 0；
   > 那時單位**還在戰場上、還會移動、還能被打**，戰備會標「戰鬥不能」。不要把 0 讀成已殲滅。

6. **統裁介入**（總監／白軍）
   右上角進「白軍控制台」：切換到任一陣營的視角看他們看得到什麼、暫停／續行、
   回放到任一快照點、注入狀況（斷橋、增援、天氣…）、開關各軍自編編組的權限。

7. **檢討**
   推演結束（達成勝負條件會自動收場）後進 AAR：逐事件時間軸、地圖重播、
   敘事型報告。報告的每一句話都會**查核引用**——引用不存在事件的敘述會被標出來。

### 0.5 想定與軍械庫

- **軍械庫**（首頁 → 武器庫）：管理武器／感測器／通信／後勤／載具的**範本**。
  範本的 `baseStats` 是資料驅動的——射程、命中率、殺傷半徑、彈種都在這裡定，引擎直接讀。
- **劇本管理／編輯器**：建立與編修想定。可匯出成 JSON 封包、也可匯入。
  編輯器裡設定的戰場範圍、tick 長度、敵我關係、交戰規則、禁射區、狀況處置腳本、勝負條件
  **都會隨開局落地到該場推演**。
- **系統設定**：推演參數（速度、耗損、補給、偵測、節奏、保真係數、每日消耗率）。
  **改了只對之後新開的局生效**——進行中的局在啟動時就把參數讀進去了，
  要套用新值得把該局封存後複製為新局。

### 0.6 幾個容易誤解的地方

- **1 tick 是多久？** 由想定宣告（範例想定是 60000 ms＝1 分鐘模擬時間）。
  牆鐘節奏另由「真實節奏壓縮比」控制（預設約 0.5 秒跑一個 tick）。
- **為什麼看不到敵人？** 迷霧是**後端強制**的——沒有偵獲就是沒有那筆資料，前端無從顯示。
  單位斷聯時，指揮所看到的是它**最後一次位置回報**，不是真實位置（畫面會標示）。
- **為什麼我的令被拒絕？** 預檢會給逐項原因。常見的是超出射程、視線被地形遮蔽、
  彈藥不足、目標在禁射區、或該局的交戰規則禁用了那項武器。
- **火力任務打不到人？** 面射擊打的是座標，命中靠散布。**沒有前觀時散布加倍**——
  這不是壞掉，是準則上的差別：教範裡的火力效能數字假設的是經過修正的射擊。
