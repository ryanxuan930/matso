...
#### WP-C4 環境演進：逐 tick 天氣、晝夜與照明、煙幕　★★｜golden：重錄（天氣快照語意變更）　✅ 2026-07-30（C4a 晝夜 + C4b 天氣 tick 化 + C4c 煙幕，三卡全數完成）
...
> 5. **未竟**：照明彈（ILLUM，需要局部短暫的光照覆寫實體）、前端晝夜呈現、C4b。
>
> **C4c（煙幕）✅ 已完成**（worklog: `docs/worklog/obscurants.md`）。與規格不同的裁決：
> 1. **煙是 LOS 的布林覆寫不是係數**：煙不是「看得比較模糊」而是遮蔽。做成 0.3 之類的
>    係數會讓「隔著煙幕狙擊」變成機率低但可行的事。
> 2. **煙存成 `MapFeature(kind="SMOKE")` 不進熱狀態**：熱狀態是 unit 鍵值的，塞 pseudo-unit
>    會讓每個 `hot.get_all()` 消費端都得學會忽略它。存 MapFeature 免費得到持久化與前端圖層。
> 3. **`blocks_los()` 簽名裡沒有陣營**——煙是雙面的，這是結構性保證（有測試釘住）。
> 4. **雷達/聲學/SIGINT 穿得過煙**（只對 `needs_los` 感測器生效）。
> 5. **未竟**：前端畫煙、COP 發煙下令 UI、`purge_expired_smoke` 未掛 pre_tick。
>    ~~風向漂移要先擴 weather 契約~~ ← **這句是錯的，已在 C4b 更正**：風場一直在契約裡
>    （`WeatherCell.wind_ms`/`wind_dir_deg`），是 core 沒讀。`drift()` 已實作（見 C4b）。
>
> **C4b（天氣逐 tick 刷新）✅ 已完成**（worklog: `docs/worklog/weather-tick.md`）。
> 開工前查證推翻了規格的兩個前提：
> 1. **插件的 RPC 早就是 tick-aware**（`GetWeatherRequest.sim_tick`）——是 core 只呼叫一次
>    且永遠傳 0。缺的不是能力，是「每隔 N tick 再問一次」。
> 2. **風場早就在契約裡**——是 core 的 `_cell_from_proto` 只讀 `cell.effects` 把風丟掉了。
>    **不需要改契約**，改的是 core 的讀法。
> 3. **golden 不必重錄**（整張 C4 標的「重錄」在三卡都不成立）：刷新間隔預設 0 ＝永不刷新
>    ＝既有的單一快照行為。實測 8 個 golden 未動。
> 4. **風向是「來向」不是「去向」**：北風＝0 度＝從北方吹來，煙往南走。直接拿它當移動方位角
>    的錯誤在畫面上完全合理（煙有在動），只有對著風標看才會發現反了。
> 5. **未竟**：`WeatherMode.REPLAY` 未接（刷新本身是決定性的，但換插件/LIVE 模式會漂）、
>    `drift()` 未掛進查詢路徑、想定/UI 不能設刷新間隔。
...
#### WP-C7 後勤體系化：補給類別、消耗、修復、整補　★★★｜golden：重錄　✅ 2026-07-30（C7.1/C7.2/C7.3 三卡全數完成）

> **C7.1（補給類別最小集）✅ 已完成**（worklog: `docs/worklog/supply-classes.md`）。
> 與規格不同的裁決：
> 1. **不把 Class III/V 搬進新體系**——油料（#84）與彈藥（#44）已經有能用的模型與測試，
>    為了「體系整齊」重寫只換到大改與回歸風險。本卡只補真正缺的 I 與 IX，
>    並提供共用的水位語義讓四類共用。
> 2. **golden 不必重錄**：消耗率預設全 0、`supply` 缺鍵回**空 dict**（不是「全部 0」，
>    後者會讓每個既有單位看起來都在斷補）、`tick_supply` 對空 dict 直接回 None
>    ——既有局一個熱狀態鍵都不會被寫。實測 8 個 golden 未動。
> 3. **`capacity <= 0` ＝未編制不是空的**：未編制的類別不消耗、不觸發再訂購、不扣效能，
>    否則每個單位都會為它沒有的東西不斷申請補給。
> 4. **按經過 tick 補算不每 tick 扣**：每 tick 扣會累積浮點誤差且回滾後對不起來。
> 5. **未竟**：LOGISTICS capacity 的類別命名與 Class 編號不一致（要改契約）、
>    想定/ORBAT 不能宣告 `supply` 初值、前端不顯示水位、Class IX 消費端在 C7.3。
>
> **C7.2（再訂購水位與補給線）✅ 已完成**（worklog: `docs/worklog/supply-lines.md`）。
> 四個決定形狀的裁決：
> 1. **補給點是 `MapFeature(kind="SUPPLY_POINT")`**（理由同煙幕）：免費得到持久化、
>    地圖圖層與**已經存在的敵我可見性語義**。
> 2. **撥交是「拉」不是「推」**：推送要求補給點知道全場有誰、誰缺什麼（那是全知），
>    而且**會讓「補給線被切斷」變得無法表達**（推送不需要路徑）。
> 3. **打掉補給點用既有的摧毀語義**——面射擊命中時呼叫 `destroy_at()`，
>    不另造一套「攻擊建物」的裁決（那會變成第二套傷害模型）。
> 4. **只拉自己陣營的補給點**：盟軍共用是後勤協定問題不是物理問題，預設不共用較保守。
> 5. **驗收條文達成**：打掉補給點後下游單位水位不再回升（有端到端測試）。
> 6. **未竟**：補給車運輸（規格的 MISSION `MOVE_MARCH` 複用；本卡只做庫存與撥交）、
>    `RESUPPLY_VOUCHER`/審批鏈開關、補給點不能從想定/UI 建立、前端不畫補給點。
>
> **C7.3（修復與人員補充）✅ 已完成**（worklog: `docs/worklog/refit.md`）。
> 1. **三個前提缺一不整補**：在補給點半徑內、附近無敵軍（5 km，落實「前線不整補」）、
>    有 Class IX（且料件是**硬上限**）。
> 2. **遭襲判定用壓制度不用戰力下降**——被射擊就累積壓制（WP-C1），那是最直接且已經存在的
>    訊號；用戰力下降判會慢一拍（要真的被打掉人才算），而整補該在第一發子彈打過來時就停。
> 3. **中斷時計時歸零**：不歸零的話被打斷再回來會把中斷期間也算成整補時間，
>    那等於「一邊挨打一邊修車」。
> 4. **第一個 tick 只計時不修**——「絕非申請後直接恢復戰力」的字面落實。
> 5. **敵軍距離用真值是刻意的**：紅線 3 管的是玩家的迷霧不是物理；這條規則的效果是
>    **限制自己**不是給予資訊優勢，事件也只說「附近有敵軍」不說是誰在哪。
> 6. **驗收條文達成**：修復中的單位遭襲即中斷整補（端到端測試）。
> 7. **未竟**：人員補充速率未與裝備修復分開（要先有 #48 單位編成組成）、
>    想定不能設修復率與安全距離、前端不顯示整補狀態。
...
#### WP-C9 友軍誤傷語意修正　★｜golden：不動（預設關）　✅ 2026-07-30（後端 + 前端 affordance）
...
> 7. **前端 affordance 已補**（2026-07-30，Backlog 清倉）：`allow_fratricide` 原本**沒有任何
>    API 回它**——後端放行了，操作員還是點不到。`SessionSummary` 加欄 → COP 把盟軍列進 ENGAGE
>    下拉（標「⚠ …（友軍）」）、瞄準中可點選友軍、送出前要勾一次確認（換目標即自動失效）。
>    途中發現**另一個既有缺陷**：白軍全局視角下 `observerFaction` 為空 → `isFriendly` 對所有
>    單位回 true（那是「全部以友軍外型呈現」的渲染語義，不是「都是我的盟友」），於是白軍的
>    ENGAGE 下拉**一直是空的**。改用 `isAlly`（要有觀測陣營才談得上誤傷）一併修掉。
> 8. **未竟**：AI 結構上仍不可能誤傷
...
#### WP-E2 認證強化　★★　✅ 2026-07-30

> **已完成**（worklog: `docs/worklog/auth-hardening.md`）。
> 1. **四項裡有一項已經做完了**：JWT secret 生產強制早就有（`ensure_production_safe`）
>    **而且真的有被呼叫**（`main.py:47`）——這次不是「寫好沒接」，我查過呼叫端才敢說。
> 2. **登出在此之前是 no-op**：端點註解自己寫著「伺服器不維護黑名單」，
>    於是撿到 refresh token 的人照樣能一直換發新的 access。現在真的撤銷。
> 3. **輪替 + 重用偵測**才是 rotation 的價值：把「偷到 token」從「可以無限續期」
>    變成「最多用一次，而且會被發現」。
> 4. **鎖定時回的錯誤與密碼錯誤完全一樣**——分開回會洩漏「這個帳號存在」。
> 5. **沒有 jti 的舊 token 略過撤銷但仍有效**（強制失效會在部署當下把所有人踢掉）。
> 6. **未竟**：完整的 token 家族撤銷（需簽發時登記每一張）、撤銷表無清理排程、
>    鎖定門檻寫死在程式。
...
#### WP-F1 SPEC_INGEST 實作：文檔→RAG 語料管線　★★★　🟡 最小切片 ✅ 2026-07-30

> **最小切片已完成**（worklog: `docs/worklog/rag-ingest.md`）。
> 1. **取不到模型是「回 None」不是「拋」**：模型不在是**部署現實**不是程式錯誤
>    （語料長期不足是本專案的設計前提，`AI_BARE` 本來就要能跑）。拋例外會讓 ingest CLI
>    在沒有模型的機器上根本跑不起來。
> 2. **但降級一定要看得見**，三處同時標示：`describe_embedder()` 的旗標、CLI stdout、
>    以及 `corpus_manifest.json` 記下當時的嵌入器——事後看到一批品質差的檢索結果，
>    沒有那一欄就無從判斷是語料不好還是嵌入器降級了。**無聲降級會讓人信任它的引用。**
> 3. **hit@k 而非更精緻的指標**：不需要相關性分級與大批標註，5 條 QA 對就給得出訊號。
>    ⚠ **`total=0` 不算通過**——「沒有東西可測」與「測了而且過了」是兩件事。
> 4. **未竟（多為部署決定不是程式決定）**：`sentence-transformers` 與 bge-m3 模型檔
>    未納入 air-gapped 部署資產（`load_bge_m3()` 已就緒，設 `MATSO_BGE_M3_PATH` 即可用）；
>    PDF/DOCX parse 未接進 CLI（屬 O9 群）；**retrieval QA 對還沒有任何一批**；
>    G5 引用查核待語料非空。
...
C1 ✅ 壓制/姿態（2026-07-31）→ C3 ✅ 乘駐車/隊形（2026-07-31）→ C2 ✅ 障礙工兵（2026-07-30）
      → C9 ✅ 誤傷語意（2026-07-30，後端 + 前端 affordance）
