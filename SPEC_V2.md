...
#### WP-C4 環境演進：逐 tick 天氣、晝夜與照明、煙幕　★★｜golden：重錄（天氣快照語意變更）　🟡 C4a ✅ + C4c ✅ 2026-07-30（晝夜、煙幕）；C4b 天氣 tick 化未做
...
> 5. **未竟**：風向漂移（`CellEffects` **沒有風場**，要先擴 weather 契約，宜與 C4b 一起）、
>    前端畫煙、COP 發煙下令 UI、`purge_expired_smoke` 未掛 pre_tick。
...
C4 環境演進：C4a ✅ 晝夜 + C4c ✅ 煙幕（2026-07-30）→ C4b 天氣 tick 化
C7 後勤體系（三卡）
