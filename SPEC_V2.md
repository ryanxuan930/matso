...
#### WP-C4 環境演進：逐 tick 天氣、晝夜與照明、煙幕　★★｜golden：重錄（天氣快照語意變更）
...
| 卡 | 內容 | 驗收 |
|----|------|------|
| G1 | **cop.vue 拆分**：以既有區塊邊界抽 composables（`useOrdering`/`useMapEditor`/`useWidgets`/`useEquipMgr`）＋子元件（OrderPanel/MapEditorPanel/UnitCard）；MapCanvas props 收斂（50 個→分組 config 物件） | 拆後 cop.vue < 800 行；行為零變更（既有 e2e 全綠 + 手測清單） |
